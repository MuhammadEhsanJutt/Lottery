...
describe("lottery contract",function()
{
    ...
    let _eventPromise;
    let _message;
    //no need to give gaslimit to hardhat blockchain as it automatically sets this 
    ...
    async function buyLottery(_maxLotteriesOneCanBuy)
    {
        ...
        if(!_winnerPicked)
        {
            ...
            //lets buy lottery    
            let tx = await _lottery.connect(_allAddress[_user]).buyLottery({value:_amountToBuy});
    ...
    describe("initial stage",function()
    {
        ...
        it("should check balance of contract",async function()
        {
            ...
            await expect(_lottery.connect(_allAddress[1]).getBalance()).to.be.revertedWith("only owner is allowed.");
        });
        ...
        it("should check verify list of withdrawals lotteries which must be 0/undefined/null etc at initial",async function()
        {
            ...
            await expect(_lottery.connect(_allAddress[1]).listOfWithdrawLottries()).to.be.revertedWith("only owner is allowed.");
        });
    });
    ...
    describe("initialize lottery",function()
    {
        ...
        it("lets check out if other than owner try to initiate the lottery is it reverted",async function()
        {
            ...
            await expect(_lottery.connect(_allAddress[1]).initiateLottery(_priceOfLottery,_maxParticipantsToPickWiner)).to.be.revertedWith("only owner is allowed.");
        });
        ...
        it("should provide inapporiate data so lottery reverted (isValidLottery).",async function()
        {
            ...
            await expect(_lottery.connect(_allAddress[0]).initiateLottery(_priceOfLottery,_maxParticipantsToPickWiner)).to.be.revertedWith("price of lottery must be greater than 0 Wei.");
            _priceOfLottery=225;//valid
            _maxParticipantsToPickWiner=2;//invalid under value
            await expect(_lottery.connect(_allAddress[0]).initiateLottery(_priceOfLottery,_maxParticipantsToPickWiner)).to.be.revertedWith("participants must be between 3-10000000.");
            _priceOfLottery=225;//valid
    ...
    describe("detail of lottery", function()
    {
        ...
        it("should check verify complete active details of lottery which must match",async function()
        {
            ...
            initiateLottery(_priceOfLottery,_maxParticipantsToPickWiner);
            ...
            {
                _message=_message+`${priceOfLottery} || ${maxParticipantsToPickWiner} || Participant: ${participant} \n`;
            }
            fnOutput(_message);
        ...
        it("should check withdraw lotteries details",async function()
        {
            ...
            //... dots are used to extract all variables
            const [...withdrawLottries] = await _lottery.listOfWithdrawLottries();
    ...
    describe("transfer funds", function()
    {
        ...
        {
            await expect(_lottery.connect(_allAddress[1]).transfer()).to.be.revertedWith("only owner is allowed.");         
        });
        it("should verify owner is not allowed to withdraw funds if someone buy ticket", async function()
        {
            ...
            await expect(_lottery.transfer()).to.be.revertedWith("someone buyed lottery, so you are not allowed.");         
        });
        it("should verify owner is able to successfuly withdraw", async function()
        {
            ...
            var tx = await _lottery.transfer();
